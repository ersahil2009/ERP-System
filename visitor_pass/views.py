...
def pass_edit(request, pk):
    vgp = get_object_or_404(VisitorGatePass, pk=pk)
    # Block editing once visitor has checked out
    if vgp.status == 'checked_out':
        messages.error(request, 'Checked-out passes cannot be edited.')
        return redirect('visitor_pass:pass_detail', pk=pk)
    # Only creator or admin can edit
    if vgp.created_by != request.user and not request.user.is_superuser:
        messages.error(request, 'Access Denied.')
        return redirect('visitor_pass:pass_list')
    form = VisitorGatePassForm(request.POST or None, instance=vgp)
    if request.method == 'POST' and form.is_valid():
        form.save()
        from accounts.models import AuditLog
        AuditLog.log(request, 'vgp_edit', 'VGP', f'VGP edited: {vgp.pass_number} by {request.user.username}')
        messages.success(request, f'Visitor Gate Pass {vgp.pass_number} updated.')
        return redirect('visitor_pass:pass_detail', pk=pk)
    return render(request, 'visitor_pass/pass_form.html', {
        'form': form, 'title': f'Edit Visitor Gate Pass — {vgp.pass_number}', 'vgp': vgp, 'is_edit': True,
    })
...
def print_pass(request, pk):
    ...
    return render(request, 'visitor_pass/vgp_print.html', {
        'vgp': vgp,
        'in_time':  fmt_time(vgp.in_time),
        'out_time': fmt_time(vgp.actual_out_time or vgp.expected_out_time),
        'date_str': '%02d.%02d.%d' % (vgp.visit_date.day, vgp.visit_date.month, vgp.visit_date.year),
    })


@login_required
def print_pass_pdf(request, pk):
    vgp = get_object_or_404(VisitorGatePass, pk=pk)
    buf = BytesIO()
    ...
    class CheckboxLabel(_Flowable):
        ...
        def draw(self):
            c = self.canv
            x = 0
            for checked, label in self._items:
                box_size = 7
                c.setLineWidth(0.6)
                c.setStrokeColor(colors.black)
                c.setFillColor(colors.white)
                c.rect(x, 1, box_size, box_size, fill=1, stroke=1)
                if checked:
                    c.setStrokeColor(colors.black)
                    c.setLineWidth(1)
                    c.line(x+1.5, 4, x+3, 6)
                    c.line(x+3, 6, x+5.5, 2)
                x += box_size + 2
                c.setFillColor(colors.black)
                c.setFont('Helvetica', 8)
                c.drawString(x, 1.5, label)
                x += c.stringWidth(label, 'Helvetica', 8) + 8
    mat_cat_para = CheckboxLabel([
        (vgp.material_category == 'non_returnable', 'Non-Returnable'),
        (vgp.material_category == 'returnable',     'Returnable'),
    ], COL2)
    ...
    main_pass = Table(
        [[header], [Spacer(1, 2*mm)], [body_row], [contact_table]],
        colWidths=[USABLE_W],
    )
