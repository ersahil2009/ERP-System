...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Work Permit
        try:
            from work_permit.models import WorkPermit, PermitApprovalStage, PermitComment, PermitRenewal
            data['work_permits'] = json.loads(
                dj_serializers.serialize('json', WorkPermit.objects.all())
            )
            data['wp_approval_stages'] = json.loads(
                dj_serializers.serialize('json', PermitApprovalStage.objects.all())
            )
            data['wp_comments'] = json.loads(
                dj_serializers.serialize('json', PermitComment.objects.all())
            )
            data['wp_renewals'] = json.loads(
                dj_serializers.serialize('json', PermitRenewal.objects.all())
            )
        except Exception as e:
            self.stderr.write(f'Work Permit backup failed: {e}')
