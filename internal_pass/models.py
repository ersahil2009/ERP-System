...
APPROVAL_WORKFLOW = {
    ...
    # Workflow 2: HOD creates → President/Plant Head → HR → Security
    'department_hod': [
        ('President / Plant Head Approval', 'president_plant_head'),
        ('HR Approval', 'hr'),
        ('Security Approval', 'security'),
    ],
    # Workflow 3: HR creates → President/Plant Head → Security (skip HR self-approval)
    'hr': [
        ('President / Plant Head Approval', 'president_plant_head'),
        ('Security Approval', 'security'),
    ],
    # Workflow 4: President/Plant Head creates → Management → Security
...
def get_workflow_stages(employee_role):
    # Fall back to 'employee' workflow for any role not explicitly mapped
    return APPROVAL_WORKFLOW.get(employee_role, APPROVAL_WORKFLOW['employee'])
