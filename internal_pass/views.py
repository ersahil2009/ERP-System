...
from .models import InternalGatePass, GatePassApproval, get_workflow_stages, BYPASS_ROLES, APPROVAL_WORKFLOW
...
def _create_approval_stages(gate_pass):
    ...
    # Resolve effective role: find first role with non-empty workflow
    all_roles = [gate_pass.employee.role] + list(gate_pass.employee.get_additional_roles())
    role = 'employee'  # default fallback
    for r in all_roles:
        if r in APPROVAL_WORKFLOW:
            workflow = APPROVAL_WORKFLOW[r]
            if workflow:  # non-empty = has stages
                role = r
                break
    # If no role has stages, use employee as fallback
    if not APPROVAL_WORKFLOW.get(role, []):
        role = 'employee'
    
    gate_pass.approvals.all().delete()
    stages = get_workflow_stages(role)
    for i, (label, approver_role) in enumerate(stages):
        GatePassApproval.objects.create(
            gate_pass=gate_pass,
            stage=i,
            stage_label=label,
...
def _advance_workflow(gate_pass, request=None):
    ...
    stages = list(gate_pass.approvals.all())
    if not stages:
        # No workflow stages — direct approval
...
def _can_user_act(user, gate_pass):
    ...
    # No actions allowed on returned passes
    ...
    # For bypass roles (management, security), they can act if no workflow stages exist
    if user.has_role('management') or user.has_role('security'):
        if gate_pass.approvals.count() == 0:  # No workflow stages = direct approval
            return gate_pass.status in ('pending', 'in_progress')
...
def pass_create(request):
    ...
    # Get workflow steps for display - use same logic as _create_approval_stages
    from .models import get_workflow_stages, APPROVAL_WORKFLOW
    all_roles = [request.user.role] + list(request.user.get_additional_roles())
    role = 'employee'  # default fallback
    for r in all_roles:
        if r in APPROVAL_WORKFLOW:
            workflow = APPROVAL_WORKFLOW[r]
    ...
    stages = get_workflow_stages(role)
    workflow_steps = [label for label, _ in stages] if stages else ['Direct Approval (no workflow stages)']
    return render(request, 'internal_pass/pass_form.html', {
        'form': form, 'title': 'New Internal Gate Pass',
...
def pass_edit(request, pk):
    ...
    # Only the employee or admin can edit
    if gp.employee != request.user and not request.user.is_superuser:
        messages.error(request, 'Access Denied.')
        return redirect('internal_pass:pass_list')
    # Only allow editing non-final statuses
    if gp.status in ('rejected', 'returned'):
        messages.error(request, 'Rejected and returned passes cannot be edited.')
        return redirect('internal_pass:pass_detail', pk=pk)

    prev_status = gp.status
    form = InternalGatePassForm(request.POST or None, instance=gp)
    if request.method == 'POST' and form.is_valid():
        gp = form.save(commit=False)
        # If pass was returned, keep it returned — just update details, no re-approval
        if prev_status == 'returned':
            gp.status = 'returned'
            gp.save()
            from accounts.models import AuditLog
            AuditLog.log(request, 'igp_edit', 'IGP', f'IGP edited (returned): {gp.pass_number} by {request.user.username}')
            messages.success(request, f'Gate Pass {gp.pass_number} updated.')
            return redirect('internal_pass:pass_detail', pk=pk)
        # For pending/in_progress/approved — reset and re-run workflow
        gp.status = 'pending'
        gp.approver = None
        gp.approved_at = None
        gp.approval_remarks = ''
        ...
        gp.current_stage = 0
        gp.save()
        # Reset all approval stages and re-run workflow
        gp.approvals.all().delete()
        _create_approval_stages(gp)
        _advance_workflow(gp, request=request)
        from accounts.models import AuditLog
        AuditLog.log(request, 'igp_edit', 'IGP', f'IGP edited & resubmitted: {gp.pass_number} by {request.user.username}')
        messages.success(request, f'Gate Pass {gp.pass_number} updated and resubmitted for approval.')
        return redirect('internal_pass:pass_detail', pk=pk)
    
    # Get workflow steps for display - use same logic as _create_approval_stages
    from .models import get_workflow_stages, APPROVAL_WORKFLOW
    all_roles = [gp.employee.role] + list(gp.employee.get_additional_roles())
    role = 'employee'  # default fallback
    for r in all_roles:
        if r in APPROVAL_WORKFLOW:
            workflow = APPROVAL_WORKFLOW[r]
    ...
    workflow_steps = [label for label, _ in stages] if stages else ['Direct Approval (no workflow stages)']
    return render(request, 'internal_pass/pass_form.html', {
        'form': form, 'title': f'Edit Gate Pass — {gp.pass_number}',
        'gp': gp, 'is_edit': True, 'workflow_steps': workflow_steps,
    })
...
def pass_approve(request, pk):
    ...
    # Superuser/admin can always act; others must match the pending stage role
    is_admin = request.user.is_superuser or request.user.has_role('administrator')
    if not is_admin and not _can_user_act(request.user, gp):
        messages.error(request, 'You are not authorized to act on this gate pass at this stage.')
        return redirect('internal_pass:pass_detail', pk=pk)

    if gp.status not in ('pending', 'in_progress'):
        messages.error(request, 'This gate pass is not pending approval.')
        return redirect('internal_pass:pass_detail', pk=pk)

    # Handle bypass roles (admin/management/security) with no workflow stages
    pending_stage = gp.approvals.filter(status='pending').first()
    if not pending_stage and (is_admin or request.user.has_role('management') or request.user.has_role('security')):
        # Direct approval for bypass roles
        ...
        if request.method == 'POST' and form.is_valid():
            ...
            if action == 'rejected':
                ...
                from accounts.models import AuditLog
                AuditLog.log(request, 'igp_reject', 'IGP', f'IGP rejected: {gp.pass_number} (direct) by {request.user.username}')
                ...
                _notify_approvers_and_admins(
                    ...
                    description='Your Internal Gate Pass has been rejected',
                    related_id=str(gp.id), related_module='IGP', recipient_employee=gp.employee
                )
            else:
                gp.status = 'approved'
                gp.approval_remarks = remarks
                ...
                gp.approved_at = timezone.now()
                gp.save()
                _send_status_mail(gp, 'approved')
                from accounts.models import AuditLog
                AuditLog.log(request, 'igp_approve', 'IGP', f'IGP approved: {gp.pass_number} (direct) by {request.user.username}')
                messages.success(request, f'Gate Pass {gp.pass_number} approved.')
                _notify_approvers_and_admins(
                    notification_type='igp_approved',
                    title=f'IGP {gp.pass_number} Approved',
                    description='Your Internal Gate Pass has been approved',
                    related_id=str(gp.id), related_module='IGP', recipient_employee=gp.employee
                )
            
            return redirect('internal_pass:pass_detail', pk=pk)
        
        # Show direct approval form
        return render(request, 'internal_pass/pass_approve.html', {
            'gp': gp, 'form': form, 'stage': None,
            'stages': gp.approvals.all(), 'is_direct_approval': True
        })
    
    if not pending_stage:
        messages.error(request, 'No pending approval stage found.')
        return redirect('internal_pass:pass_detail', pk=pk)
...
def mark_returned(request, pk):
    gp = get_object_or_404(InternalGatePass, pk=pk)
    # Allow marking return for approved passes regardless of expected_return_time (on leave)
    if gp.status != 'approved':
        messages.error(request, 'Only approved passes can be marked as returned.')
        return redirect('internal_pass:pass_detail', pk=pk)
...
def export_report(request):
    ...
    headers = ['Pass No', 'Employee', 'Emp Code', 'Department', 'Purpose', 'Destination',
               'Date', 'Out Time', 'Exp. Return', 'Actual Return', 'Transport', 'Status',
               'Approval 1 — Name', 'Approval 1 — Stage', 'Approval 1 — Status', 'Approval 1 — Time',
               'Approval 2 — Name', 'Approval 2 — Stage', 'Approval 2 — Status', 'Approval 2 — Time',
               'Approval 3 — Name', 'Approval 3 — Stage', 'Approval 3 — Status', 'Approval 3 — Time',
               ]
        ...
        for i in range(3):
            if i < len(stages):
                ...
                apv_cols += [
                    ...
                    s.stage_label,
                    s.status.upper(),
                    s.acted_at.strftime('%d-%m-%Y %H:%M') if s.acted_at else '',
                ]
            else:
                apv_cols += ['', '', '', '']
        ws.append([
