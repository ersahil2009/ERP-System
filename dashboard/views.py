...
from work_permit.models import WorkPermit, PERMIT_TYPE_CHOICES, PERMIT_TYPE_COLORS, PERMIT_TYPE_ICONS
...
def index(request):
    ...
    ctx = {
        ...
        # ── Work Permit ──
        'wp_total':   WorkPermit.objects.count(),
        'wp_active':  WorkPermit.objects.filter(status='approved').count(),
        'wp_pending': WorkPermit.objects.filter(status='pending').count(),
        'wp_draft':   WorkPermit.objects.filter(status='draft').count(),
        'wp_closed':  WorkPermit.objects.filter(status='closed').count(),
        'wp_expired': WorkPermit.objects.filter(status='expired').count(),
        'recent_wp':  WorkPermit.objects.select_related('requested_by').order_by('-created_at')[:10],
        'wp_active_list': WorkPermit.objects.filter(status='approved').order_by('start_datetime')[:5],
        'wp_type_cards': [
            {'code': code, 'label': label,
             'color': PERMIT_TYPE_COLORS.get(code, '#374151'),
             'icon': PERMIT_TYPE_ICONS.get(code, 'bi-file-earmark-check'),
             'count': WorkPermit.objects.filter(permit_type=code).count()}
            for code, label in PERMIT_TYPE_CHOICES
        ],
        'wp_by_type': [
            {'label': label, 'color': PERMIT_TYPE_COLORS.get(code, '#374151'),
             'count': WorkPermit.objects.filter(permit_type=code).count()}
            for code, label in PERMIT_TYPE_CHOICES
            if WorkPermit.objects.filter(permit_type=code).exists()
        ],
    ...
    try:
        ...
        ctx['today_quote'] = DailyQuote.today()
    except Exception:
        ctx['today_quote'] = None
