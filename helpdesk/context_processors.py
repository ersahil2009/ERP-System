def helpdesk_context(request):
    ...
    try:
        from work_permit.models import WorkPermit
        user = request.user
        if user.is_superuser or getattr(user, 'perm_wp_approve', False) or user.role in ('administrator', 'management', 'president_plant_head', 'department_hod'):
            ctx['pending_wp_count'] = WorkPermit.objects.filter(status='pending').count()
        else:
            ctx['pending_wp_count'] = 0
    except Exception:
        pass
