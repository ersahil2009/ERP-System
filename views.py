...
def _build_perm_summary(emp):
    ...
    MODULES = [
        ...
        ('Work Permit',  [('View', 'perm_wp_view'), ('Write', 'perm_wp_write'), ('Approve', 'perm_wp_approve'), ('Delete', 'perm_wp_delete')]),
        ('Reports',      [('IGP', 'perm_reports_igp'), ('VGP', 'perm_reports_vgp'), ('MGP', 'perm_reports_mgp'), ('GRV', 'perm_reports_grv'), ('Audit', 'perm_reports_audit'), ('WP', 'perm_wp_report')]),
    ]
    result = []
...
def system_settings(request):
    ...
    if request.method == 'POST':
        ...
        if action == 'save_series':
            ...
            setting.grv_prefix = request.POST.get('grv_prefix', 'GRV').strip().upper()
            setting.save()
            AuditLog.log(request, 'settings_change', 'Settings', f'Pass prefixes updated: IGP={setting.igp_prefix}, VGP={setting.vgp_prefix}, TKT={setting.tkt_prefix}, MGP={setting.mgp_prefix}, GRV={setting.grv_prefix}')
            messages.success(request, 'Pass series prefixes updated successfully.')

        elif action == 'reset_grv':
            new_num = int(request.POST.get('grv_reset_number', 1))
            setting.grv_next_number = new_num
            setting.save(update_fields=['grv_next_number'])
            AuditLog.log(request, 'settings_change', 'Settings', f'GRV series reset to {setting.grv_prefix}-{new_num:05d}')
            messages.success(request, f'GRV series reset to {setting.grv_prefix}-{new_num:05d}.')
        ...
        elif action == 'reset_db_grv':
            confirm_text = request.POST.get('confirm_text', '').strip()
            if confirm_text == 'DELETE GRV':
                from grievance.models import Grievance, GrievanceComment
                GrievanceComment.objects.all().delete()
                count = Grievance.objects.count()
                Grievance.objects.all().delete()
                setting.grv_next_number = 1
                setting.save(update_fields=['grv_next_number'])
                AuditLog.log(request, 'settings_change', 'Settings', f'GRV database reset — {count} records deleted by {request.user.username}')
                messages.success(request, f'Grievance database cleared. {count} records deleted and series reset to {setting.grv_prefix}-00001.')
            else:
                messages.error(request, 'Confirmation text did not match. GRV data was NOT deleted.')

        elif action == 'save_wp_prefix':
            setting.wp_prefix = request.POST.get('wp_prefix', 'WP').strip().upper()
            setting.save(update_fields=['wp_prefix'])
            AuditLog.log(request, 'settings_change', 'Settings', f'WP prefix updated to {setting.wp_prefix}')
            messages.success(request, f'Work Permit prefix updated to {setting.wp_prefix}.')

        elif action == 'save_wp_settings':
            setting.wp_prefix  = request.POST.get('wp_prefix', 'WP').strip().upper()
            setting.wp_doc_ref = request.POST.get('wp_doc_ref', 'PTW-F-001').strip()
            setting.wp_rev     = request.POST.get('wp_rev', '01').strip()
            setting.save(update_fields=['wp_prefix', 'wp_doc_ref', 'wp_rev'])
            AuditLog.log(request, 'settings_change', 'Settings',
                         f'WP settings updated: prefix={setting.wp_prefix}, doc_ref={setting.wp_doc_ref}, rev={setting.wp_rev}')
            messages.success(request, f'Work Permit settings saved. Prefix: {setting.wp_prefix} | Doc Ref: {setting.wp_doc_ref} | Rev: {setting.wp_rev}')

        elif action == 'reset_wp':
            new_num = int(request.POST.get('wp_reset_number', 1))
            setting.wp_next_number = new_num
            setting.save(update_fields=['wp_next_number'])
            AuditLog.log(request, 'settings_change', 'Settings', f'WP series reset to {setting.wp_prefix}-{new_num:05d}')
            messages.success(request, f'WP series reset to {setting.wp_prefix}-{new_num:05d}.')

        elif action == 'reset_db_wp':
            confirm_text = request.POST.get('confirm_text', '').strip()
            if confirm_text == 'DELETE WP':
                from work_permit.models import WorkPermit, PermitComment, PermitExtension
                PermitExtension.objects.all().delete()
                PermitComment.objects.all().delete()
                count = WorkPermit.objects.count()
                WorkPermit.objects.all().delete()
                setting.wp_next_number = 1
                setting.save(update_fields=['wp_next_number'])
                AuditLog.log(request, 'settings_change', 'Settings', f'WP database reset — {count} records deleted by {request.user.username}')
                messages.success(request, f'Work Permit database cleared. {count} records deleted and series reset to {setting.wp_prefix}-00001.')
            else:
                messages.error(request, 'Confirmation text did not match. WP data was NOT deleted.')
    ...
    from work_permit.models import WorkPermit as WorkPermitModel
    from accounts.models import NotificationPermission

    wp_last_obj = WorkPermitModel.objects.order_by('-id').first()
    wp_total    = WorkPermitModel.objects.count()
    wp_last     = wp_last_obj.permit_number if wp_last_obj else None
    ...
    context = {
        ...
        'grv_last': GrievanceModel.objects.order_by('-id').first(),
        'grv_total': GrievanceModel.objects.count(),
        'wp_total': wp_total,
        'wp_last': wp_last,
        ...
        'wp_workflow_variants': [
            {'key': 'base', 'label': 'Standard (No Isolation, No MOC)', 'steps': [
                {'num': 1, 'label': 'Initiator', 'dept': ''},
                {'num': 2, 'label': 'Issuer — Dept HOD', 'dept': ''},
                {'num': 3, 'label': 'Custodian', 'dept': 'Process & Production'},
                {'num': 4, 'label': 'Safety Officer', 'dept': 'HSEF'},
                {'num': 5, 'label': 'Co-Permit (Final)', 'dept': ''},
            ]},
            {'key': 'isolation', 'label': 'Isolation Checked', 'steps': [
                {'num': 1, 'label': 'Initiator', 'dept': ''},
                {'num': 2, 'label': 'Issuer — Dept HOD', 'dept': ''},
                {'num': 3, 'label': 'Custodian', 'dept': 'Process & Production'},
                {'num': 4, 'label': 'Isolator', 'dept': 'Electrical'},
                {'num': 5, 'label': 'Issuer — Electrical HOD', 'dept': 'Electrical'},
                {'num': 6, 'label': 'Safety Officer', 'dept': 'HSEF'},
                {'num': 7, 'label': 'Co-Permit (Final)', 'dept': ''},
            ]},
            {'key': 'isolation_moc', 'label': 'Isolation + MOC Checked', 'steps': [
                {'num': 1, 'label': 'Initiator', 'dept': ''},
                {'num': 2, 'label': 'Issuer — Dept HOD', 'dept': ''},
                {'num': 3, 'label': 'Custodian', 'dept': 'Process & Production'},
                {'num': 4, 'label': 'Isolator', 'dept': 'Electrical'},
                {'num': 5, 'label': 'Issuer — Electrical HOD', 'dept': 'Electrical'},
                {'num': 6, 'label': 'Safety Officer', 'dept': 'HSEF'},
                {'num': 7, 'label': 'MOC', 'dept': 'President/Plant Head'},
                {'num': 8, 'label': 'Co-Permit (Final)', 'dept': ''},
            ]},
            {'key': 'moc', 'label': 'MOC Only Checked', 'steps': [
                {'num': 1, 'label': 'Initiator', 'dept': ''},
                {'num': 2, 'label': 'Issuer — Dept HOD', 'dept': ''},
                {'num': 3, 'label': 'Custodian', 'dept': 'Process & Production'},
                {'num': 4, 'label': 'Safety Officer', 'dept': 'HSEF'},
                {'num': 5, 'label': 'MOC', 'dept': 'President/Plant Head'},
                {'num': 6, 'label': 'Co-Permit (Final)', 'dept': ''},
            ]},
            {'key': 'renewal', 'label': 'Permit Renewal Workflow', 'steps': [
                {'num': 1, 'label': 'Initiator (Request)', 'dept': ''},
                {'num': 2, 'label': 'Issuer — Dept HOD', 'dept': 'Acknowledge only'},
                {'num': 3, 'label': 'Custodian', 'dept': 'Process & Production — Approval'},
                {'num': 4, 'label': 'Safety Officer', 'dept': 'HSEF — Acknowledge only'},
            ]},
        ],
    }
    ...
    from accounts.models import DailyQuote
    from django.utils import timezone as _tz
    _today_day = _tz.localdate().timetuple().tm_yday
    _quotes = list(DailyQuote.objects.all())
    for q in _quotes:
        q.is_today = (q.day_number == _today_day)
    context['daily_quotes_preview'] = _quotes
...
def user_rights_matrix(request):
    ...
    PERM_COLS = [
        ...
        ('Grievance', [
            ('perm_grv_view',   'View'),
            ('perm_grv_write',  'Raise'),
            ('perm_grv_manage', 'Manage'),
        ]),
        ('Work Permit', [
            ('perm_wp_view',    'View'),
            ('perm_wp_write',   'Write'),
            ('perm_wp_approve', 'Approve'),
            ('perm_wp_delete',  'Delete'),
        ]),
        ('Reports', [
            ('perm_reports_igp',   'IGP'),
            ('perm_reports_vgp',   'VGP'),
            ('perm_reports_mgp',   'MGP'),
            ('perm_reports_grv',   'GRV'),
    ...
    if request.GET.get('export') == 'excel':
        ...
        col = 4
        mod_colors = {'Dashboard': navy, 'Accounts': blue, 'IGP': green, 'VGP': purple, 'MGP': PatternFill('solid', fgColor='B7410E'), 'Grievance': PatternFill('solid', fgColor='7B4BB3'), 'Work Permit': PatternFill('solid', fgColor='DC2626'), 'Reports': PatternFill('solid', fgColor='1A6B3C')}
    ...
    if request.GET.get('export') == 'pdf':
        ...
        mod_color_map = {
            ...
            'Grievance': _c.HexColor('#7b4bb3'),
            'Work Permit': _c.HexColor('#dc2626'),
...
def daily_quotes(request):
    ...
    if request.method == 'POST':
        ...
        if action == 'import_quotes' and request.FILES.get('quotes_file'):
            import openpyxl
            try:
                wb = openpyxl.load_workbook(request.FILES['quotes_file'])
                ws = wb.active
                imported = 0
                for row in ws.iter_rows(min_row=2, values_only=True):
                    if not row[0]:
                        continue
                    day = int(row[0])
                    quote = (row[1] or '').strip()
                    author = (row[2] or '').strip()
                    if 1 <= day <= 365 and quote:
                        DailyQuote.objects.update_or_create(
                            day_number=day,
                            defaults={'quote': quote, 'author': author}
                        )
                        imported += 1
                messages.success(request, f'{imported} quotes imported successfully.')
            except Exception as e:
                messages.error(request, f'Import failed: {e}')
            return redirect('accounts:daily_quotes')

        elif action == 'delete_all':
            count = DailyQuote.objects.count()
            DailyQuote.objects.all().delete()
            AuditLog.log(request, 'settings_change', 'Settings', f'All {count} daily quotes deleted')
            messages.success(request, f'{count} quotes deleted.')
            return redirect('accounts:daily_quotes')

    quotes = DailyQuote.objects.all()
    today_quote = DailyQuote.today()

    return render(request, 'accounts/daily_quotes.html', {
        'quotes': quotes,
...
@login_required
def download_quotes_template(request):
    import openpyxl
    from openpyxl.styles import Font, PatternFill, Alignment
    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = 'Daily Quotes'
    headers = ['Day (1-365)*', 'Quote*', 'Author']
    hfill = PatternFill(start_color='2A2A86', end_color='2A2A86', fill_type='solid')
    for col, h in enumerate(headers, 1):
        cell = ws.cell(row=1, column=col, value=h)
        cell.font = Font(bold=True, color='FFFFFF')
        cell.fill = hfill
    ...
    ws.append([1, 'The only way to do great work is to love what you do.', 'Steve Jobs'])
    ws.append([2, 'Success is not final, failure is not fatal.', 'Winston Churchill'])
    ws.append([3, 'Believe you can and you\'re halfway there.', 'Theodore Roosevelt'])
    buf = BytesIO()
    wb.save(buf)
    buf.seek(0)
    resp = HttpResponse(buf, content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    resp['Content-Disposition'] = 'attachment; filename="daily_quotes_template.xlsx"'
    return resp
